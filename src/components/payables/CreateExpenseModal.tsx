...
export default function CreateExpenseModal({ isOpen, onClose, onSuccess }: CreateExpenseModalProps) {
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const hasItbis = document.getElementById('include_itbis') as HTMLInputElement;
    
    // Calculate ITBIS only if checkbox is checked
    const taxAmount = hasItbis?.checked ? amount * 0.18 : 0;
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const taxAmount = e.target.checked ? amount * 0.18 : 0;
                      setFormData({
                        ...formData,
                        tax_amount: taxAmount,
                        total_amount: amount + taxAmount
                    ...
                    className="h-4 w-4 rounded border-gray-600/50 bg-gray-700/50 text-emerald-500 focus:ring-emerald-500"
                  />
                  <label htmlFor="include_itbis" className="ml-2 block text-sm text-gray-300">
                    Incluir ITBIS (18%)
                  </label>
                </div>
              </div>
